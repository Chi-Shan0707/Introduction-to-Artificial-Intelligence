"""
Bonus3 统一训练脚本（Q-learning）。

目标：
1) 训练阶段可选“观测加噪”（train_obs_noise_std）。
2) 推理阶段可选“邻域平滑/置信门控平滑”（smooth_* 参数）。

推荐实验口径：
- baseline 与 inference_smooth 使用同一个 checkpoint（仅推理规则不同）。
- obsnoise_train 使用独立训练得到的 checkpoint（训练分布不同）。
"""
...
def parse_smooth_dims(smooth_dims: str) -> Tuple[int, ...]:
    ...
    if name not in mapping:
        raise ValueError(
            f"Unknown smooth_dims={smooth_dims!r}. Valid keys: {sorted(mapping)}"
        )
    return mapping[name]
...
class Agent:
    def __init__(
        ...
        # 推理阶段平滑参数
        smooth_mode: str = DEFAULT_SMOOTH_MODE,          # none | weighted | confidence
        smooth_alpha: float = DEFAULT_SMOOTH_ALPHA,      # 中心状态权重
        smooth_radius: int = DEFAULT_SMOOTH_RADIUS,      # 邻域半径
        smooth_dims: str = DEFAULT_SMOOTH_DIMS,          # 主方案：平滑杆角速度维度
        confidence_tau: float = DEFAULT_CONFIDENCE_TAU,  # 置信门控阈值
    ):
    ...
    def _one_axis_neighbor_ids(self, center_id: int) -> List[int]:
        """
        保守邻域：中心点 + 指定维度单轴扩展。
        例如 angle_angular + radius=1 时，最多 5 个状态，避免 3^4 的过平滑。
        """
    ...
    def _robust_q(self, state: np.ndarray) -> Tuple[np.ndarray, dict]:
        ...
        raise ValueError(
            "smooth_mode must be one of: none, weighted, confidence. "
            f"Got {self.smooth_mode!r}."
        )
            ...
            pickle.dump(
                {
                    ...
                    # 存一份默认推理参数，评估时仍允许通过 init kwargs 覆盖
    ...
    def load_model(self, filepath: str):
        """加载 Q 表，但保留构造函数传入的推理参数（便于评估时切换策略）。"""
        # 先缓存构造时传入的推理参数
        smooth_mode = self.smooth_mode
        smooth_alpha = self.smooth_alpha
        smooth_radius = self.smooth_radius
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Default: ./results next to this file",
    )
    args = parser.parse_args()

    if args.results_dir is None:
        base_dir = Path(__file__).resolve().parent
        model_dir = (base_dir.parent / CHECKPOINT_DIR).resolve()
        figure_dir = base_dir / RESULTS_DIR / "figures"
    else:
        model_dir = Path(args.results_dir)
        figure_dir = Path(args.results_dir)
    model_dir.mkdir(parents=True, exist_ok=True)
    figure_dir.mkdir(parents=True, exist_ok=True)

    ckpt = model_dir / f"{args.output_prefix}_model.pkl"
    curve = figure_dir / f"{args.output_prefix}_training_curve.png"
