"""Bonus 3 (Approach 5): TD-Error-Guided Adaptive Discretization.

Dynamic bin boundaries adapt based on cumulative TD errors. Bins become finer
in regions where TD errors are high (indicating poor value estimation), and
coarser where TD errors are low (well-estimated regions).
"""
...
class Agent:
    ...
    def _adapt_boundaries(self):
        ...
        for dim in range(4):
            ...
            for b in range(MAX_DIGITIZE_NUM):
                ...
                if np.any(mask) and np.sum(self.visit_counts[mask]) > 0:
                    weights = self.visit_counts[mask]
                    td_avg = np.average(self.td_errors[mask], weights=weights)
                elif np.any(mask):
                    td_avg = np.mean(self.td_errors[mask])
                else:
                    td_avg = 0.0
                td_per_bin.append(float(td_avg))

            # 累积密度：高 TD 误差区域密度更高 -> 边界更密
            cum_density = [0.0]
            for b in range(MAX_DIGITIZE_NUM):
                width = boundaries[b + 1] - boundaries[b]
                dens = 1.0 + TD_ALPHA * td_per_bin[b]
    ...
    def save_model(self, filepath):
        ...
        print(f"自适应 TD 误差离散化模型已保存到: {filepath}")
    ...
    def load_model(self, filepath):
        ...
        print(f"自适应 TD 误差离散化模型已加载: {filepath}")
    ...
    def train(self, epoch=EPOCHS, save_path=None):
        ...
        for i in range(epoch):
            if i > 0 and i % ADAPT_INTERVAL == 0:
                print(f"\n--- Episode {i}: Adapting boundaries (TD-error-guided) ---")
...
def main():
    ...
    env = Environment()
    print("开始训练自适应 TD 误差离散化 Q-learning...")
    record = env.train(
        epoch=EPOCHS,
        save_path=os.path.join(SAVE_DIR, 'q_learning_bonus3_adaptive_tderror_model.pkl')
    )

    plt.figure(figsize=(10, 5))
    plt.plot(np.arange(len(record)), record)
    plt.title('Adaptive TD-Error-Guided Discretization Training Curve')
    plt.xlabel('Episode')
    plt.ylabel('Steps')
    plot_path = os.path.join(SAVE_DIR, 'q_learning_bonus3_adaptive_tderror_curve.png')
