...
def plot_lines(rows, out_png: Path):
    ...
    for ax, test in zip(axes, tests):
        ...
        ax.set_title("Initial Perturbation" if test == "init_perturb" else "Observation Perturbation")
        ax.set_xlabel("scale")
        ax.grid(alpha=0.25)
    axes[0].set_ylabel("mean steps")
    handles, labels = axes[1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=3, frameon=True)
    fig.suptitle("Bonus3 Robustness Comparison (4 scales)", y=1.03)
    plt.tight_layout()
...
def main():
    ...
    for path in (eval_dir, figures_dir, tables_dir, models_dir):
        path.mkdir(parents=True, exist_ok=True)

    py = sys.executable

    baseline_ckpt = project_dir / "checkpoints" / "q_learning_model.pkl"
    obsnoise_ckpt = models_dir / f"{args.obsnoise_prefix}_model.pkl"
    ...
    for agent in agents:
        for scale in args.scales:
            ...
            init_report = eval_dir / f"init_{agent['name']}_ps{tag}.json"
            ...
            obs_report = eval_dir / f"obs_{agent['name']}_ns{tag}.json"
            obs_summary = eval_job(
                py,
                project_dir,
                obs_report,
                [
                    "Bonus_3/scripts/evaluate_obs_perturb.py",
                    "--agent-class",
                    agent["agent_class"],
                    "--agent-init-kwargs",
                    agent["kwargs"],
    ...
    out_json = tables_dir / "bonus3_summary_4scales.json"
    out_csv = tables_dir / "bonus3_summary_4scales.csv"
    out_png = figures_dir / "bonus3_robustness_4scales.png"
    out_box_svg = figures_dir / "bonus3_boxplot_4scales.svg"
