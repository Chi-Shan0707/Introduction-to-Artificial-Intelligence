...
def plot_diagnostics(summary: list[dict], out_path: Path) -> None:
    variants = [row["variant"] for row in summary]
    x = np.arange(len(variants))

    time_limit = np.array([row.get("time_limit_count", 0) for row in summary], dtype=float)
    cart_pos = np.array([row.get("cart_pos_count", 0) for row in summary], dtype=float)
    pole_angle = np.array([row.get("pole_angle_count", 0) for row in summary], dtype=float)
    mean_steps = np.array([row["mean_steps"] for row in summary], dtype=float)
    ...
    axes[0].bar(x, time_limit, color=colors["time_limit"], label="time limit")
    axes[0].bar(x, cart_pos, bottom=time_limit, color=colors["cart_pos"], label="cart position")
    axes[0].bar(x, pole_angle, bottom=time_limit + cart_pos, color=colors["pole_angle"], label="pole angle")
    axes[0].set_title("Terminal Reasons")
    ...
    axes[1].bar(x, mean_steps, color="#9d8df1")
    axes[1].set_title("Mean Episode Length")
    axes[1].set_ylabel("steps")
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(variants, rotation=25, ha="right")
    axes[1].grid(axis="y", alpha=0.25)
    for idx, value in enumerate(mean_steps):
        axes[1].text(idx, value + max(mean_steps) * 0.015, f"{value:.0f}", ha="center", va="bottom", fontsize=9)

    fig.suptitle("Bonus3 Failure Diagnostics", y=0.99)
    fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.94])
    fig.savefig(out_path, dpi=170, bbox_inches="tight", pad_inches=0.14)
    plt.close(fig)
...
def main() -> None:
    ...
    plot_diagnostics(summary, out_dir / "failure_diagnostics.png")

    print("Done. Failure diagnostics written to:", out_dir)
    print("Plot:", out_dir / "failure_diagnostics.png")
    for row in summary:
        print(
            row["variant"],
