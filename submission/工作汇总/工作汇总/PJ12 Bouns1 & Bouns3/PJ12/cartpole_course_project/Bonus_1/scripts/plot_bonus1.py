...
def main():
    ...
    for sc, tag in zip(scales, tags):
        reports[sc] = {
            "Q-learning": load_report(results_dir / f"qlearning_eval_{tag}.json"),
            "REINFORCE": load_report(results_dir / f"reinforce_eval_{tag}.json"),
            "MCTS": load_report(results_dir / f"mcts_eval_{tag}.json"),
        }
    ...
    fig.tight_layout()
    fig.savefig(results_dir / "bonus1_seed_step_boxplot_4in1.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(results_dir / "bonus1_seed_step_hist_4in1.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(results_dir / "bonus1_seed_step_by_seed_4in1.png", dpi=150)
    plt.close(fig)

    # 2) 定量对比表（CSV）
    table_csv = results_dir / "bonus1_quant_table.csv"
    rows = []
    for sc in scales:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "bonus1_perturb_mean_steps.png", dpi=150)
    plt.close()

    # 5) 速率对比图（读 timing_summary.csv）
    timing_csv = results_dir / "timing_summary.csv"
    if timing_csv.exists():
        ...
        if grouped:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(results_dir / "bonus1_compute_rate.png", dpi=150)
            plt.close()
