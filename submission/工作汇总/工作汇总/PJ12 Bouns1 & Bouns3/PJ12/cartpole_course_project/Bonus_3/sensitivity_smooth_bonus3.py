"""
Bonus 3: inference smoothing sensitivity tests.

Two complementary checks are supported:
1) strength: scan smooth_alpha x confidence_tau x smooth_radius for the
   chosen smoothing dimension.
2) dims: compare four single-state-dimension smoothing choices at fixed
   strength settings.

The script calls the existing evaluate.py interface, so it only tests initial
state perturbation. Per-step observation noise is handled by evaluate_obs_perturb.py.
"""
